...
Tests are defined as functions that receive `helpers` and return an attribute set with checks:

```nix
nix-tests.runTests {
  "test name" = helpers: rec {
    ctx = 42;

    "check name" = helpers.isEq ctx 42;
  };

  "group name" = {
    "nested test" = helpers: {
      "check name" = helpers.isTrue true;
    };
  };
}
```
...
All available via `helpers` parameter:
